...
function AMFRSerializeWorker() {
	...
	this.writeUTF = function(str, asAMF) {
		var bytearr, c, count, i, strlen, utflen;
		strlen = str.length;
		utflen = 0;
		count = 0;
		for (i = 0; i < strlen; i++) {
...
function AMFRDeserializeWorker(payload) {
	...
	this.readLong = function() {
		return ((this.read() * Math.pow(2, 56))
				+ ((this.read() & 255) * Math.pow(2, 48))
				+ ((this.read() & 255) * Math.pow(2, 40))
				+ ((this.read() & 255) * Math.pow(2, 32))
				+ ((this.read() & 255) * Math.pow(2, 24))
				+ ((this.read() & 255) << 16) + ((this.read() & 255) << 8) + ((this
				.read() & 255) << 0));
	};
	...
	this.call = function(message, onResult, onStatus) {
		...
		req.onreadystatechange = function() {
			if (req.readyState === 1) {
				...
				var contLength = req.getResponseHeader("content-length");
